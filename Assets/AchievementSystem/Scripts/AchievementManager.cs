...
public class AchievementManager : MonoBehaviour
{
    ...
    public void Unlock(int Index)
    {
        if (!States[Index].Achieved)
        {
            ...
            if (AchievementList[Index].UnspoilAfterUnlock != "")
            {
                UnSpoil(AchievementList[Index].UnspoilAfterUnlock);
            }
    ...
    public void UnSpoil(string Key)
    {
        int index = -1;
        do
        {
            if (index+1 >= AchievementList.Count) index = -1;
            else
            {
                index = AchievementList.FindIndex(index+1, x => x.DisplayName.Equals(Key));
            }

            if (index != -1) States[index].Unlocked = true;
        } while (index != -1);
    }
    ...
    public void AddAchievementProgress(string Key, float Progress)
    {
        ...
        do
        {
            index = FindAchievementIndex(Key, index + 1);
            if (index != -1) AddAchievementProgress(index, Progress);
        } while (index != -1);
    }
    /// <summary>
    /// Adds the input amount of progress to an achievement. Clamps achievement progress to its max value.
    ...
    public void SaveAchievementState()
    {
        for (int i = 0; i < States.Count; i++)
        {
            ...
            PlayerPrefs.SetInt("Unlocked" + i, States[i].Unlocked ? 1 : 0);
        }
        PlayerPrefs.Save();
    }
    /// <summary>
    ...
    public void LoadAchievementState()
    {
        ...
        for (int i = 0; i < AchievementList.Count; i++)
        {
            ...
            if(PlayerPrefs.HasKey("StateProgress" + i))
            {
                ...
                NewState.Unlocked = Convert.ToBoolean(PlayerPrefs.GetInt("Unlocked" + i));
            }
            States.Add(NewState);
        }
    }
    ...
    private int FindAchievementIndex(string Key, int startIndex = 0)
    {
        if (startIndex >= AchievementList.Count) return -1;
        return AchievementList.FindIndex(startIndex, x => x.Key.Equals(Key));
    }
    /// <summary>
    ...
    {
        if (DisplayAchievements && (!AchievementList[Index].Spoiler || States[Index].Unlocked) || States[Index].Achieved)
        {
            //If not achieved
